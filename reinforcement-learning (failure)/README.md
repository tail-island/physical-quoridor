...
で、結果なのですが、私が試せた範囲だと惨敗でした……。`pip install -r requirements.txt`して`python evaluate.py`すると、かなり気の抜けたプレイをご覧いただけると思います（私はWSL2上のUbuntuで実験しました。ごめんなさい、Windowsで動くかは未検証です）。
